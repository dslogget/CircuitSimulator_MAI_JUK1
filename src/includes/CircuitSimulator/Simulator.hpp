...
public:
   ...
   SimulationEnvironment( std::string netlistPath )
       : netlistPath( netlistPath ), luPair( 0 ), scratchSpace( 0, 0 ), solutionMat( 0, 0 ) {
#ifdef WITH_MATLAB
      ...
      while ( !netlist.eof() ) {
         ...
         switch ( lType ) {
            ...
            case LineType::Capacitor:
               if ( line[ 1 ] == 'N' ) {
                  NLCapacitor< VT >::addToElements( line, elements, numNodes, numCurrents, numDCCurrents );
               } else {
            ...
            case LineType::CurrentSource:
               if ( line[ 1 ] == 'N' ) {
                  NLCurrentSource< VT >::addToElements( line, elements, numNodes, numCurrents, numDCCurrents );
               } else {
            ...
            case LineType::VoltageSource:
               switch ( static_cast< SourceType >( line[ 1 ] ) ) {
                  case SourceType::TimeSeries:
                     TimeSeriesVoltageSource< VT >::addToElements( line, elements,
                                                                   numNodes,
                                                                   numCurrents,
                                                                   numDCCurrents );
                     break;
                  case SourceType::Sinusoidal:
                     SinusoidalVoltageSource< VT >::addToElements( line, elements,
                                                                   numNodes,
                                                                   numCurrents,
                                                                   numDCCurrents );
                     break;
                  default:
                     VoltageSource< VT >::addToElements( line, elements, numNodes,
                                                         numCurrents,
                                                         numDCCurrents );
                     break;
               }
...
#ifndef WITH_MATLAB
                  if ( line[ 2 ] == 'F' ) {
                     std::throw( "ERROR: Matlab not available at compile time" );
                  }
#endif
                  SParameterBlockVF< VT >::addToElements( line, elements, numNodes,
                                                          numCurrents, numDCCurrents );
               } else {
            ...
            case LineType::Transistor:
               if ( line[ 1 ] == 'N' ) {
                  BJTN< VT >::addToElements( line, elements, numNodes, numCurrents, numDCCurrents );
               } else if ( line[ 1 ] == 'P' ) {
                  BJTP< VT >::addToElements( line, elements, numNodes, numCurrents, numDCCurrents );
               } else if ( line[ 1 ] == 'M' ) {
                  if ( line[ 2 ] == 'N' ) {
                     NLNMOS< VT >::addToElements( line, elements, numNodes, numCurrents, numDCCurrents );
                  } else {
                     std::cout << "Other Transistors not implemented yet" << std::endl;
                  }
               } else {
                  std::cout << "Other Transistors not implemented yet" << std::endl;
               }
               break;
            case LineType::Diode:
               Diode< VT >::addToElements( line, elements, numNodes, numCurrents, numDCCurrents );
               break;
         ...
         for ( nr = 0; nr < maxNR; nr++ ) {
            ...
            for ( size_t k = 0; k < solutionMat.M; k++ ) {
               ...
               if ( std::isnan( tempSoln( k, 0 ) ) ) {
                  std::cout << "Simulation Error: NaN found in solution" << std::endl;
               }
#endif
   ...
   void
   printMultipleOnGraph( std::vector< size_t > nodeVec, std::string suffix = "" ) {
#ifdef WITH_PYTHON
      namespace plt = matplotlibcpp;
      plt::figure_size( 1200, 780 );
   ...
   void dataDump() {
      ...
      for ( size_t n = 0; n < solutionMat.N; n++ ) {
...
#ifdef WITH_MATLAB
         if ( matlabDesktop ) {
            sArray[ n ][ varNames[ 0 ] ] = factory.createArray( { 1, 1 },
                                                                { n * timestep } );
         }
#endif
         for ( int i = 0; i < numNodes + numCurrents; i++ ) {
            ...
            if ( matlabDesktop ) {
               sArray[ n ]
                     [ varNames[ i + 1 ] ] = factory.createArray( { 1, 1 },
                                                                  { solutionMat( i , n  ) } );
            }
#endif
         }
