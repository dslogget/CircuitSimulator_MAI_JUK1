...
public:
   ...
   addNonLinearStampTo( Stamp< T > & stamp,
      ...
      T di_ds_d = - beta_DS * alpha_DS / std::pow( std::cosh( alpha_DS * ( u_gs - u_gd ) ), 2 );
      T di_ds_s = beta_DS * alpha_DS / std::pow( std::cosh( alpha_DS * ( u_gs - u_gd ) ), 2 );

      T i_gd = C_GD * ( 2.0 * ( u_gd - u_gd_last ) / timestep - i_gd_last / C_GD_last );
      T i_gs = C_GS * ( 2.0 * ( u_gs - u_gs_last ) / timestep - i_gs_last / C_GS_last );

      T i_d = -i_gd + i_ds;
      T i_s = -i_gs - i_ds;
      T i_g = i_gs + i_gd;

      T di_gd = dC_GD * ( 2.0 * ( u_gd - u_gd_last ) / timestep - i_gd_last / C_GD_last ) +
                2.0 * C_GD / timestep;
      T di_gs = dC_GS * ( 2.0 * ( u_gs - u_gs_last ) / timestep - i_gs_last / C_GS_last ) +
                2.0 * C_GS / timestep;
   ...
   void updateStoredState( const Matrix< T > & solutionMatrix,
      ...
      i_gd_last = C_GD * ( 2.0 * ( u_gd - u_gd_last ) / timestep - i_gd_last / C_GD_last );
      i_gs_last = C_GS * ( 2.0 * ( u_gs - u_gs_last ) / timestep - i_gs_last / C_GS_last );
