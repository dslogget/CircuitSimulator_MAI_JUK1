...
struct TimeSeriesVoltageSource : public Component< T > {
   ...
   T lerp( size_t lowIndex, T timeVal ) const {
      T diffTS = timeSeries[ ( lowIndex + 1 ) % timeSeries.size() ] - timeSeries[ lowIndex ];
      T diffTV = timeVal - timeSeries[ lowIndex ];
      T diffDS = dataSeries[ ( lowIndex + 1 ) % timeSeries.size() ] - dataSeries[ lowIndex ];
   ...
   void updateStoredState( const Matrix< T > & solutionMatrix,
                           ...
                           size_t sizeG_A ) {
      while ( std::fmod( currentSolutionIndex * timestep, timeSeries.back() ) > timeSeries[ ( lastTimeSeriesIndex + 1 ) % timeSeries.size() ] || ( lastTimeSeriesIndex != 0 && std::fmod( currentSolutionIndex * timestep, timeSeries.back() ) < timeSeries[ ( lastTimeSeriesIndex - 1 ) % timeSeries.size() ] ) ) {
   ...
   addToElements( const std::string & line, CircuitElements< T > & elements,
      ...
      std::regex timeSeriesVoltageSourceRegex = generateRegex( "VT", "n n w s", true, false );
      ...
      std::regex_search( matches.suffix().first, line.cend(), matches, endOfLineRegex );
   ...
   void
   readInTimeSeries( T timescale, const std::string & seriesPath ) {
