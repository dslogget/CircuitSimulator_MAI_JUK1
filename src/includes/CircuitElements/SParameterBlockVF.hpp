...
struct SParameterBlockVF : public Component< T > {
   ...
   std::complex< T >
   history_p( const size_t p, const Matrix< T > & solutionMatrix,
      ...
      for ( size_t c = 0; c < numPorts; c++ ) {
         ...
         if ( currentSolutionIndex > 1 ) {
            toRet += port[ p ].from[ c ].nu *
                     awave_p( c, solutionMatrix, currentSolutionIndex - 2, sizeG_A );
         }
      }
      ...
      for ( size_t p = 0; p < port.size(); p++ ) {
         ...
         for ( size_t c = 0; c < port.size(); c++ ) {
            if ( c != p ) {
               ...
               stamp.G( stamp.sizeG_A + curr,
                        stamp.sizeG_A + port[ c ].current - 1 ) +=
                        std::real( -z_ref * port[ p ].alpha[ c ] );
            }
         }
      }
   }
   ...
   void updateStoredState( const Matrix< T > & solutionMatrix,
      ...
      for ( size_t p = 0; p < numPorts; p++ ) {
         for ( size_t c = 0; c < numPorts; c++ ) {
            for ( size_t rho = 0; rho < port[ p ].from[ c ].numPoles; rho++ ) {
               port[ p ].from[ c ].x[ rho ] = port[ p ].from[ c ].x[ rho ] * port[ p ].from[ c ].exp_alpha[ rho ] +
                                              port[ p ].from[ c ].lambda_p[ rho ] * awave_p( c, solutionMatrix, currentSolutionIndex, sizeG_A ) +
                                              port[ p ].from[ c ].mu_p[ rho ] * awave_p( c, solutionMatrix, currentSolutionIndex - 1, sizeG_A );
               if ( !firstOrder ) {
                  port[ p ].from[ c ].x[ rho ] += port[ p ].from[ c ].nu_p[ rho ] * awave_p( c, solutionMatrix, currentSolutionIndex - 2, sizeG_A );
               }
            }
         }
      }
      ...
      for ( size_t p = 0; p < numPorts; p++ ) {
         ...
         for ( size_t c = 0; c < numPorts; c++ ) {
            if ( c == p ) {
               ...
               port[ p ].alpha[ c ] = port[ p ].from[ c ].lambda + port[ p ].from[ c ].remainder;
   ...
   void addDCAnalysisStampTo( Stamp< T > & stamp,
      ...
      for ( size_t p = 0; p < port.size(); p++ ) {
         for ( size_t c = 0; c < port.size(); c++ ) {
            for ( size_t rho = 0; rho < port[ p ].from[ c ].numPoles; rho++ ) {
               xSum[ p * numPorts + c ] += - ( port[ p ].from[ c ].lambda_p[ rho ] + port[ p ].from[ c ].mu_p[ rho ] ) / (port[p].from[c].exp_alpha[rho] - 1.0);
            }
      ...
      for ( size_t p = 0; p < port.size(); p++ ) {
         ...
         stamp.G( stamp.sizeG_A + curr,
                  stamp.sizeG_A + curr ) += std::real( -z_ref * ( 1.0 + xSum[ p * numPorts + p ] ) * beta );
         ...
         for ( size_t c = 0; c < port.size(); c++ ) {
            if ( c != p ) {
               ...
               stamp.G( stamp.sizeG_A + curr,
                        stamp.sizeG_A + port[ c ].current - 1 ) +=
                        std::real( -z_ref * xSum[ p * numPorts + c ] * beta );
            }
         }
      }
   }
