...
struct CircuitElements {
   ...
   CircuitElements( size_t numNodes = 0, size_t numCurrents = 0, size_t numDCCurrents = 0 )
       : staticStamp( numNodes, numCurrents ),
         dynamicStamp( numNodes, numCurrents ),
   ...
   /// @return A reference to the cached stamp.
   Stamp< T > &
   generateDynamicStamp( const Matrix< T > & solutionMatrix,
   ...
   Stamp< T > &
   generateCompleteStamp( SolutionStage stage,
   ...
   void updateDCStoredState( const Matrix< T > & solutionVector, size_t numCurrents ) {
      for ( const auto & component : staticElements ) {
         component->updateDCStoredState( solutionVector, dcStamp.sizeG_A, numCurrents );
      }
      for ( const auto & component : dynamicElements ) {
         component->updateDCStoredState( solutionVector, dcStamp.sizeG_A, numCurrents );
      }
