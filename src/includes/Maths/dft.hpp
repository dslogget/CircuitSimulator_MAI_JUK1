...
nthRootOfUnity(int numerator, size_t denominator) {
    return std::exp(std::complex<T>(0, -2 * std::numbers::pi * numerator /
...
fft(const std::vector<T> & inputData) {
    std::vector<std::complex<T> > result(inputData.size(), std::complex<T>(0, 0));
    std::vector<std::complex<T> > scratch(inputData.size(), std::complex<T>(0, 0));
