...
struct _interpreter {
   ...
   /* For now, _interpreter is implemented as a singleton since its currently not
      ...
      patching the python source code or starting a separate process for each. [1]
      Furthermore, many python objects expect that they are destructed in the same
      thread as they were constructed. [2] So for advanced usage, a `kill()` function
      is provided so that library users can manually ensure that the interpreter is
      constructed and destroyed within the same thread.
...
// Sanity checks; comment them out or change the numpy type below if you're compiling
// on a platform where they don't apply
...
bool bar( const std::vector< Numeric > & x, const std::vector< Numeric > & y,
   ...
   PyObject * res = PyObject_Call( detail::_interpreter::get().s_python_function_bar,
                                   plot_args, kwargs );
...
struct is_callable_impl< true, T > {
   ...
   static std::true_type test(
       ... ); // use a variadic function to make sure (1) it accepts everything and
...
template< typename T >
struct is_callable {
   // dispatch to is_callable_impl<true, T> or is_callable_impl<false, T> depending
   // on whether T is of class type or not
...
struct plot_impl< std::true_type > {
   ...
   bool operator()( const Iterable & ticks, const Callable & f,
      ...
      // We could use additional meta-programming to deduce the correct element type
      // of y, but all values have to be convertible to double anyways
      std::vector< double > y;
      for ( auto x : ticks )
         y.push_back( f( x ) );
